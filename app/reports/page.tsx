...
import { TrendingUp, Users, Mail, DollarSign, Shield, FileText, Download, Filter } from 'lucide-react';
...
const Reports = () => {
  ...
  return (
    <div className="p-8 bg-black min-h-screen">
                ...
                <tbody>
                  {marketingCampaigns.map((campaign, idx) => (
                    <tr key={idx} className="border-b border-gray-800 hover:bg-[#2A2A2A] transition-colors">
                      ...
                      <td className="py-4 px-4 text-right text-white">{campaign.opened.toLocaleString()}</td>
                      <td className="py-4 px-4 text-right text-white">{campaign.clicked.toLocaleString()}</td>
      ...
      {activeCategory === 'compliance' && (
        <div className="space-y-6">
          ...
          <div className="bg-[#1A1A1A] border border-gray-800 rounded-xl p-6">
            ...
            <div className="space-y-3">
              {['DNC List Review', 'TCPA Compliance', 'Document Retention', 'Fair Lending Review'].map((item, idx) => (
                <div key={idx} className="flex justify-between items-center p-4 bg-[#2A2A2A] rounded-lg">
      ...
      {/* Other Sections */}
      {['financial', 'custom'].includes(activeCategory) && (
